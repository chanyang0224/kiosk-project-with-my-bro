...
<body>
	...
	<main class="container">
		<c:forEach items="${icetea}" var="icetea">
			<% 
				KioskVO icetea = (KioskVO)pageContext.getAttribute("icetea");
				String img = icetea.getImg();
	...
	<script>
		...
		if(window.sessionStorage){
			// 아래의 로직을 수행
			// null값 확인을 위한 cnt값을 먼저 확인
			cnt = Number(sessionStorage.getItem("cnt"));
			// cnt==0이라면(arrProduct 등의 값들이 null이라면)
			if(cnt==0) {
				// 새롭게 값을 생성
				// 주문한 상품명을 보관하는 배열
			...
			// null값이 아닌 값들을 전송받았다면
			else {
				// session으로 부터 정보를 전달 받음
